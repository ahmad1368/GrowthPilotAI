...
/// Drives the `FlutterBridge` JS channel (Issue #220/#221/#222) — no
/// real Next.js/React Flow `/canvas-lite` app exists yet, so this loads
/// the bundled placeholder page instead (see PR notes).
class VisualModelingBridgeController extends GetxController {
  ...
  final exportReady = Rxn<ExportPayload>();
  ...
  void _onBridgeMessage(JavaScriptMessage message) {
    ...
    final exportPayload = ParseExportPayload.call(parsed);
    if (exportPayload != null) exportReady.value = exportPayload;
  }
}
