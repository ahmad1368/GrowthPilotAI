...
/// Owns the inventory item list (Issue #435), refreshing it locally after
/// each quick-add insert. Rendering itself is [InventoryReportView]'s job.
class InventoryReportBody extends StatefulWidget {
  final List<InventoryItemEntity> initialItems;

  const InventoryReportBody({super.key, required this.initialItems});
...
class _InventoryReportBodyState extends State<InventoryReportBody> {
  ...
  Future<void> _addItem() async {
    final item = await showInventoryItemDialog(context);
    if (item == null) return;
  ...
  Widget build(BuildContext context) =>
      InventoryReportView(items: _items, onAddItem: _addItem);
}
