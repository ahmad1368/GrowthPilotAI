...
/// Loads every non-archived conversation as a most-recent-first
/// [ConversationSummary] list (Issue #72), resolving each row's
/// linked-transaction amount (Issue #69) and ACTION_CARD payload (Issue
/// #73). Archived conversations (Issue #76) are excluded — swiping/batch
/// archiving removes a row from this list until it's un-archived.
class LoadInboxSummaries {
  static List<ConversationSummary> call(
    ...
    return conversations.getAll().where((c) => !c.isArchived).map((conversation) {
