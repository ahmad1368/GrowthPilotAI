...
- `flutter analyze` رو در این پایپلاین اصلاً اجرا نکن (کند و پرهزینه‌ست). `flutter test` هم اجرا نکن (نه کل سوئیت، نه فایل‌های تست جدید) — فقط فایل تست رو بنویس و کنارش بذار؛ اجرای تست‌ها یک تصمیم/تسک جدا از کاربره که خودش هروقت خواست دستی انجام می‌ده. اگر لازم دیدی درستیِ کد رو بسنجی، فقط یک بررسی سریع و هدفمند (مثلاً خوندن دوباره‌ی فایل تغییر‌یافته یا یک دستور کوچیک محلی) کافیه، نه اجرای ابزارهای سنگین کل‌پروژه.
- QA بصری (اجرای emulator/شبیه‌ساز، اسکرین‌شات، تست دستی UI) هم در این پایپلاین اصلاً انجام نده — کند و ناپایدار بوده. اگر کاربر بعداً خواست، دستور `/emulator-qa` جدا برای همین کار هست.
...
5. **پیاده‌سازی** — الزامات معماری (چک‌لیست، بدون توضیح اضافه در حین کار):
   ...
   - قبل از باز کردن PR، اگه فایلی که تغییر دادی احتمال داره روی وب کامپایل نشه، حتماً `flutter build web --release` رو یک‌بار لوکال امتحان کن؛ خطای کامپایل وب را رفع کن (مگر اینکه از قبل مستند شده باشه که این خطا پیش‌زمینه‌ای و نامرتبط با تغییرات توئه).
6. **تست**: فایل Unit test جدا با نام صریح در `test/` برای منطق جدید (بدون اجرا).
7. **بررسی نهایی**: `flutter analyze` و هر نوع «UI analyze» اینجا اجرا نکن. اگر لازم دیدی، فقط یه بررسی سریع و هدفمند انجام بده (نه یک ابزار کند و کل‌پروژه). اگر فایل‌های تغییر‌یافته احتمال ناسازگاری با وب دارن (پلتفرم-محور/native) یک‌بار `flutter build web --release` اجرا کن؛ خطا → رفع. اجرای emulator/شبیه‌ساز یا هر نوع QA بصری اینجا لازم نیست (کاربر خودش جدا با `/emulator-qa` انجام می‌ده).
8. **کامیت/پوش/PR**: کامیت انگلیسی → push → `gh pr create --base stage` (فقط به `stage`، هرگز مستقیم به `main`؛ بدون merge خودکار) → روی همون PR ریپورت انگلیسی کوتاه:
   ```
   ## Summary
   ...
   ## Notes (open follow-ups for future issues, incl. "visual QA not run — use /emulator-qa")
   ```
