...
/// Renders the Business Compass as a pluggable report bundle (Issue #111)
/// offscreen and writes light/dark PNGs into `screenshots/` for the PR.
void main() {
  ...
  const specs = [
    ...
    ReportWidgetSpec(
        ...
        data: {'narrative': 'Your biggest gap vs. the sector is Vendor Diversity.'}),
    ReportWidgetSpec(
        ...
        body: RepaintBoundary(
          ...
          child: Container(
            ...
            child: const SingleChildScrollView(
                child: DynamicReportGallery(specs: specs)),
          ),
        ),
      ),
    ));
  ...
  testWidgets('writes light and dark report-gallery screenshots',
      (tester) async {
    await capture(tester, Brightness.light, 'report_gallery_light.png');
    await capture(tester, Brightness.dark, 'report_gallery_dark.png');
    expect(
        File('screenshots/report_gallery_light.png').existsSync(), isTrue);
    expect(File('screenshots/report_gallery_dark.png').existsSync(), isTrue);
  });
}
